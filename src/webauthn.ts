...
export async function registerBiometric(userId: string, masterPassword: string): Promise<boolean> {
  if (isRegistering) {
  ...
  try {
    ...
    const encryptedPassword = CryptoJS.AES.encrypt(masterPassword, derivedKey).toString()
...
export async function authenticateWithBiometric(): Promise<string | null> {
  ...
  try {
    ...
    const decryptedPassword = CryptoJS.AES.decrypt(encryptedPassword, derivedKey).toString(CryptoJS.enc.Utf8)
