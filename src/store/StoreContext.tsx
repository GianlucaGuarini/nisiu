...
interface StoreContextType extends Store {
  ...
  setEncryptedKey: (password: string, enableBiometric?: boolean) => Promise<void>
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const checkBiometric = async () => {
      ...
      setState(prev => ({ ...prev, biometricAvailable: available, isTrustedDevice: trusted }))
    }
    checkBiometric()
  }, [])
  ...
  const logout = useCallback(async () => {
    const auth = getAuth(firebaseApp)
    await signOut(auth)
    setState(prev => ({ ...prev, key: false, masterPassword: null, isLocked: true, passwords: [] }))
  }, [])
  ...
  const unlock = useCallback(async (password: string): Promise<boolean> => {
    ...
    if (key) {
      setState(prev => ({ ...prev, key, masterPassword: password, isLocked: false }))
      return true
    }
    return false
  ...
  const setEncryptedKey = useCallback(async (password: string, enableBiometricFlag = false) => {
    ...
    setState(prev => ({ ...prev, key, encryptedKey: key, masterPassword: password, isLocked: false }))
  }, [state.user, state.key])

  const enableBiometric = useCallback(async (): Promise<boolean> => {
    if (!state.user || !state.key || !state.masterPassword) return false
    const success = await registerBiometric(state.user.uid, state.masterPassword)
    if (success) {
      setState(prev => ({ ...prev, isTrustedDevice: true }))
    }
    return success
